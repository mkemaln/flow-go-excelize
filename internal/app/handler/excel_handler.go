...
func (h *ExcelHandler) GenerateExcel(w http.ResponseWriter, r *http.Request) {
	...
	startRow := 6
	verticalSpacing := 3 // How many rows to jump for a vertical connection
	...
	for i, shapeType := range shapeTypes {
		...
		shape := newFlowchartShape(currentShapeCell, shapeType)
		file.AddShape("Sheet1", shape)

		file.SetRowHeight("Sheet1", currentRow, 30)
		file.SetColWidth("Sheet1", currentCol, currentCol, convertLengthToPixel(100))
